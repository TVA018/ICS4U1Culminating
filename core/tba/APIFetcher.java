...
public final class APIFetcher {
    ...
    public static Event getEvent(String eventKey) {
        ...
        var rankingsJson = (List<HashMap<String, Object>>) eventRankingInfo.get("rankings");

        HashMap<Integer, Double> rankingScores = new HashMap<>();

        for(var teamRankingScoreInfo : rankingsJson) {
            ...
            rankingScores.put(teamNumber, rankingScore);
        }

        TerminalTextFormatter.println("Loading district points...", ANSIFlag.YELLOW_TEXT);
        var districtPointsJSon = (Map<String, Map<String, Integer>>) ((Map<String, Object>) fetch("/event/" + eventKey + "/district_points")).get("points");
        HashMap<Integer, Integer> districtPointsMap = new HashMap<>();

        for(var entry : districtPointsJSon.entrySet()) {
            int teamNumber = Conversions.teamNumberFromKey(entry.getKey());
            int districtPoints = entry.getValue().get("total");

            districtPointsMap.put(teamNumber, districtPoints);
        }
